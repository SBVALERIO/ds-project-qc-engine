...
def test_standalone_tbd_cell_is_flagged(tmp_path):
    ...
    findings = analyze_document(document, check_tbd_specs=True)
...
def test_tbd_specs_not_checked_by_default(tmp_path):
    pdf_path = tmp_path / "finish_sample.pdf"
    _build(str(pdf_path))
    document = load_document(str(pdf_path))
    findings = analyze_document(document)

    assert not any("Especificação não resolvida" in f.title for f in findings)
